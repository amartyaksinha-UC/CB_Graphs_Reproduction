...
```{r}
setwd("C:/Users/Aishwarya S/OneDrive - The University of Chicago/Advance Illinois/GraphsToReplicate/ACS_Data/Table2.14") 
```

```{r}
pppub21 <- read.csv("pppub21.csv")
```

```{r}
# Filter for Illinois, age, full-time employment, and relevant sectors
illinois_data <- pppub21 %>%
  filter(A_AGE > 24,
         A_WKSTAT %in% c(2,3),
         PRCOW1 %in% c(1, 2, 3, 4),
         GESTFIPS == 17)
